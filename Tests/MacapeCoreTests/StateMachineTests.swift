...
final class StateMachineTests: XCTestCase {
    ...
    func testQueuedKeyAfterMultipleHomeRowModifiersGetsAllFlags() {
        var snapshot = StateMachineSnapshot(keys: [
            key(0x00, modifier: .maskCommand, hold: 100),
            key(0x01, modifier: .maskAlternate, hold: 100),
        ])
        ...
        XCTAssertTrue(actions.contains(where: {
            if case .postKey(0x08, down: true, let flags) = $0 {
    ...
    func testPendingHomeRowKeysPromoteForPointerEvents() {
        var snapshot = StateMachineSnapshot(keys: [
            key(0x00, modifier: .maskCommand),
            key(0x01, modifier: .maskAlternate),
        ])
        let layer = LayerConfig.default
        _ = HomeRowStateMachine.handleKeyEvent(
            snapshot: &snapshot,
        ...
        guard let click = CGEvent(
            ...
            XCTFail("failed to create mouse event")
            return
        }

        _ = PointingEventModifier.applyHomeRowModifiers(
            snapshot: &snapshot,
            event: click,
            nowMs: 1020
        )
        ...
        XCTAssertEqual(snapshot.keys[0].state, .modifier)
        XCTAssertEqual(snapshot.keys[1].state, .modifier)
        XCTAssertTrue(click.flags.contains(.maskCommand))
        XCTAssertTrue(click.flags.contains(.maskAlternate))
    }
